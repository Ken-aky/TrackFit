...
export class ActivityVisualizationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (!this.logedusername) {
      console.error('Benutzername fehlt, LocalStorage leer!');
  ...
  fetchActivitySummary() : void {
    this.http.get<any[]>(`http://localhost:8080/api/v1/activity/activities-summary/${this.selectedYear}/${this.logedusername}`)
      .subscribe({
        next: (response) => {
          this.activitySummary = response;
          // Wenn die API-Daten erfolgreich geladen wurden, Diagramm rendern
          this.renderChart(response);
          console.log(`Aufgerufene API-URL: http://localhost:8080/api/v1/activity/activities-summary/${this.selectedYear}/${this.logedusername}`);
        },
        error: (error) => {
